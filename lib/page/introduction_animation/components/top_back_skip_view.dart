...
class TopBackSkipView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 定义游客按钮的滑动动画，从上到下滑入
    final _guestAnimation = Tween<Offset>(begin: Offset(2, 0), end: Offset(0, 0))
        .animate(CurvedAnimation(
          parent: animationController, // 使用传入的动画控制器
          curve: Interval(0.6, 0.8, curve: Curves.fastOutSlowIn), // 设置动画间隔和曲线
        ));
    ...
    return SlideTransition(
      ...
      child: Padding(
        padding: EdgeInsets.only(top: MediaQuery.of(context).padding.top), // 确保导航栏不会被状态栏遮挡
        child: Container(
          height: 58, // 设置导航栏的高度
          child: Padding(
